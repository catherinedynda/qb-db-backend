    ...
    if (msg.people) {
        ...
        msg.people.forEach(async (person) => {
            if (person.search(/^Me/) !== -1) {
                ...
                const member =
                    memberNames[memberIds.indexOf(msg.quote.member_id)];
...
const fixPeopleText = (people, member_id) => {
    if (people) {
        let newPeople = [];
        let memberNames = fs.readFileSync("data/member_names.txt", "utf-8");
        memberNames = memberNames.split("\n");
        ...
        people.forEach(async (person) => {
            if (person.search(/^Me/) !== -1) {
                person = memberNames[memberIds.indexOf(member_id)];
            } else if (person.search(/^My/i) !== -1) {
                const member = memberNames[memberIds.indexOf(member_id)];
                person = person.replace(/^My/i, `${member}'s`);
            }
            newPeople.push(person);
        });
    ...
    if (msg.search(/^Me/)) {
        try {
            ...
            const member = await client.query(
                'SELECT groupme_name FROM qb."Member" WHERE member_id = $1',
                [74325816]
            );
...
export { processPerson, fixPeople, fixPeopleText };
