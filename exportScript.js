...
import { fixPeople } from './person.js';
import pkg from 'pg';
...
export default async function() {
    ...
    if (err) {
        ...
        const writeStreamNicknames = fs.createWriteStream('data/nicknames.csv');
        const writeStreamQuoteNickname = fs.createWriteStream('data/quote_nickname.csv');
        ...
        writeStreamQuoteNickname.on('finish', () => console.log('Quote_Nickname CSV written'));
        writeStreamLikes.on('finish', () => console.log('Likes CSV written'));
        writeStreamNicknames.on('err', (err) => console.log(`Error writing nicknames: ${err}`));
        writeStreamQuoteNickname.on('err', (err) => console.log(`Error writing quote_nickname: ${err}`));
        writeStreamLikes.on('error', (err) => console.log(`Error writing likes: ${err}`));
            ...
            if (processedMsg) {
                ...
                fixedPeople.forEach((person) => {
                    ...
                    if (nicknames.indexOf(person) !== -1) {
                        ...
                        writeStreamQuoteNickname.write(`${processedMsg.quote.quote_id},${nicknameId}\n`);
                    } else {
